...
import com.SBTPG.TPGMobile.fragments.feepaidReport.ReportsFeesPaidFragment;
...
public class ReportAccountDisbParticularSearchListAdapter extends RecyclerView.Adapter<ReportAccountDisbParticularSearchListAdapter.ReportsViewHolder> {
    List<ReportsPerticularAccountDisbSearchNew> reportsList;
    String title;
    private ItemClickListener clickListener;
    String index;
    Context mContext;

    public ReportAccountDisbParticularSearchListAdapter(Context mContext, List<ReportsPerticularAccountDisbSearchNew> reportsList, String title) {
    ...
    @Override
    public ReportAccountDisbParticularSearchListAdapter.ReportsViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.report_list_row, parent, false);
        ReportAccountDisbParticularSearchListAdapter.ReportsViewHolder reportsViewHolder
                = new ReportAccountDisbParticularSearchListAdapter.ReportsViewHolder(itemView);
        return reportsViewHolder;
    }

    @Override
    public void onBindViewHolder(ReportAccountDisbParticularSearchListAdapter.ReportsViewHolder holder, int position) {
     /*   if (position % 2 == 0)
            holder.itemView.setBackgroundColor(Color.parseColor("#ebefef"));
        else
            holder.itemView.setBackgroundColor(Color.parseColor("#e0e8e8"));*/

        ReportsPerticularAccountDisbSearchNew reports = reportsList.get(position);
